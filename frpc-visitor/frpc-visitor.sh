...
Commands:
  ...
  stop            停止容器
  restart         重启容器
  status          查看容器状态 + visitor 代理状态
  logs            跟踪日志 (Ctrl+C 退出)
  reload          热重载配置 (需启用 Admin UI)
  update <ver>    升级版本,例如: ./frpc-visitor.sh update v0.71.0
...
case "${1:-}" in
    ...
    reload)
        ...
        docker exec "$NAME" frpc reload -c "$CONF" 2>&1 \
            && echo "重载成功" \
            || echo "重载失败: 请确认 visitor.toml 中已启用 webServer 段" >&2
        ;;
    update)
        VER="${2:-}"
        if [ -z "$VER" ]; then
